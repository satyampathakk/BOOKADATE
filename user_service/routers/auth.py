...
def login(data: LoginData, db: Session = Depends(get_db)):
    ...
    if user.registration_status != "approved":
        raise HTTPException(status_code=403, detail=f"Registration {user.registration_status or 'pending'}")
...
def verify_token(request: Request):
    ...
    try:
        ...
        return {"user_id": payload.get("sub"), "email": payload.get("email"), "token_valid": True}
